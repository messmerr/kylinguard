...
def test_公开示例管理员密码会被服务端拒绝(tmp_path, placeholder):
    with pytest.raises(ValueError, match="公开示例值"):
        create_app(Settings(
            _env_file=None,
...
async def test_草稿创建端点要求登录并严格校验合同(app):
    valid = {
        "session_id": "e" * 32,
        "mode": "full_access",
        "ttl_seconds": 60,
    ...
    async with _client(app) as client:
        unauthorized = await client.post("/api/sessions", json=valid)
        headers = await _login(client)
...
async def test_完全访问可由环境显式关闭并返回明确原因(tmp_path):
    settings = Settings(
        ...
        admin_password=PW, allow_full_access=False,
    )
...
def test_后端重启总会要求完全访问重新复验(tmp_path):
...
async def test_草稿创建先校验且冲突不会覆盖已有会话(tmp_path):
    ...
    async with _client(draft_app) as client:
        ...
        exists_after_wrong_password = draft_app.state.sessions.exists(session_id)
    ...
    assert exists_after_wrong_password is False
...
async def test_完全访问无需独立账户但仍要求管理员密码和TTL(tmp_path):
    workspace = tmp_path / "project"
    workspace.mkdir()
    settings = Settings(
        _env_file=None, db_path=str(tmp_path / "full-access.db"),
        admin_password=PW, workspace_root=str(workspace),
