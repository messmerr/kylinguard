...
globalThis.fetch = async (url, options = {}) => {
  ...
  const sessionPermissionMatch = url.match(/^\/api\/sessions\/([^/]+)\/permissions$/)
  if (sessionPermissionMatch) {
    if (permissionResponses.length) return permissionResponses.shift()
    const fixture = permissionFixture || { mode: 'ask', version: 1 }
    return Response.json({
      ...fixture,
      session_id: sessionPermissionMatch[1],
      base_mode: fixture.base_mode || (fixture.mode === 'full_access' ? 'ask' : fixture.mode),
      base_version: fixture.base_version || 1,
    })
  }
  const fullAccessMatch = url.match(/^\/api\/sessions\/([^/]+)\/full-access$/)
  if (fullAccessMatch) {
    const body = JSON.parse(options.body)
    permissionUpdateBodies.push(body)
    return Response.json({
      ...(permissionFixture || {}),
      session_id: fullAccessMatch[1],
      mode: body.enabled ? 'full_access' : 'ask',
      version: body.enabled ? body.version + 1 : 1,
      base_mode: 'ask',
      base_version: 1,
    })
  }
...
test('旧任务回放不会重新激活已经失效的确认请求', async () => {
  reset()
  sessionListFixture = [{
    id: 'old-session', title: '旧任务', busy: false,
    created_at: 1, updated_at: 2, workspace_root: '/srv/old',
  }]
  ...
  sessionEventsFixture = { events: [{
    ...
    payload: {
      thought: '准备执行',
      steps: [{
        step_id: 'old-step', tool: 'files.write_file', arguments: {},
        purpose: '写入文件', risk: 'medium',
      }],
    },
  }, {
    seq: 2,
    ...
    payload: {
      confirm_id: 'expired-confirm', step_id: 'old-step',
      step: { tool: 'files.write_file', arguments: {}, purpose: '写入文件' },
      decision: { action: 'confirm', risk: 'medium', reason: '需要确认' },
    },
  }] }
  permissionFixture = { mode: 'ask', version: 1 }
  ...
  assert.equal(chat.items.value.some((item) => item.kind === 'fatal'), false)
})

test('单条不兼容的旧版事件不会阻止其余任务记录显示', async () => {
  reset()
  sessionEventsFixture = { events: [{
    event_type: 'user_query', payload: { query: '旧任务问题' },
  }, {
    event_type: 'plan', payload: { thought: '旧版计划缺少步骤字段' },
  }, {
    event_type: 'final_answer', payload: { answer: '仍可查看的结果' },
  }] }
  permissionFixture = { mode: 'ask', version: 1 }

  await chat.loadSession('legacy-session')

  assert.equal(chat.items.value.some((item) => item.text === '旧任务问题'), true)
  assert.equal(chat.items.value.some((item) => item.text === '仍可查看的结果'), true)
  assert.equal(chat.items.value.some((item) => item.kind === 'history_warning'), true)
  assert.equal(chat.items.value.some((item) => item.kind === 'fatal'), false)
})
...
test('首条消息前不能开启任务级完全访问', async () => {
  ...
  await assert.rejects(
    chat.setChatPermissionMode('full_access'),
    /请先发送一条消息创建任务/,
  )
  assert.equal(chat.activeId.value, '')
  assert.equal(permissions.permissionContext.sessionId, '')
  ...
  assert.deepEqual(permissionUpdateBodies, [])
})
