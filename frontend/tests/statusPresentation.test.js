...
test('总览页按端点局部降级且详细状态只受状态接口影响', () => {
  ...
  assert.match(source, /refreshSystemStatus\(\{ force: forceStatus \}\)/)
...
test('审计页把任务审计与系统变更分层并使用独立系统范围接口', () => {
  const source = componentSource('../src/views/AuditView.vue')
  assert.match(source, /任务审计/)
  assert.match(source, /系统变更/)
  assert.match(source, /activeKind === 'task'/)
  assert.match(source, /v-for="session in sessions"/)
  assert.match(source, /v-for="scope in systemScopes"/)
  assert.match(source, /\/api\/audit\/scopes\/\$\{scope\.id\}\/events/)
  assert.match(source, /配置审计链校验通过/)
  assert.match(source, /scope_type: activeKind\.value/)
  assert.doesNotMatch(source, /\.\.\.systemScopes\.value, \.\.\.sessions\.value/)
})
