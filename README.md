...
自动审核 / 完全访问**四种全局审批模式；完全访问具备完整 Agent 能力，高风险模式
...
任务输入框旁可以随时查看或切换全局权限，选择会对当前及以后创建的所有任务生效。
系统首次启动默认“确认后执行”；“自动审核”
会在静态规则与独立 Reviewer 均通过后，自动执行授权范围内的可逆操作；当前
服务器默认工作目录会在范围为空时作为初始值，高风险、破坏性、越界或审核异常的
动作仍需确认。目录范围全局持久化且不再与审批模式互斥；“允许一次”和“本次会话允许”
只保存具体动作授权。“完全访问”入口默认隐藏，只能先在“权限与安全”中通过独立
醒目警告和二次输入确认将其显示；真正启用时还要再次通过另一组警告和二次输入确认。
后端会拒绝跳过入口揭示步骤的直接启用请求。开启后 Agent 可使用完整 shell、文件、
网络与进程能力，不再逐项询问。
它以界面显示的 OS 身份运行，不会自动获得 root；KylinGuard 的 LLM 密钥和
控制面配置不会下发到工具子进程。开启后会持续生效，直到手动切换权限、隐藏入口、
服务端关闭该能力或后端重启；若使用后端
...
模型服务无需登录；提供商变更仍执行版本校验并进入审计日志。完全访问仍保留明确
二次确认和审计；具体高风险操作的会话授权仍保留 TTL、单次授权和审计，因为它们会
扩大 Agent 的系统执行权限。
...
需要时仍须通过可审计的 `files.*` 工具读取，并继续遵守全局审批模式、会话动作授权和风险门控。
...
- **授权可撤销**：批准绑定会话、权限版本、能力、资源与动作指纹；支持仅一次、
  本会话和自动执行范围授权；一次授权只有收到其 grant id 的原等待步骤可消费，
...
### 完全访问（高风险，能力可用但入口默认隐藏）

完全访问不要求额外创建执行账户。开启后，Agent 在 `KG_WORKSPACE_ROOT`（默认项目根）
中拥有完整 shell、文件、网络和进程能力，不再对每个动作逐项确认。它必须先在
“权限与安全”页面单独揭示，揭示和启用各自都有不可静默跳过的两阶段确认，其中第二步
要求输入指定短语。隐藏入口会原子收回正在生效的完全访问并恢复为“确认后执行”。
界面会明确显示实际 OS 执行身份；若 `KG_EXEC_USER` 留空，就是后端当前身份。
...
这不是开启完全访问的前置条件。设置 `KG_ALLOW_FULL_ACCESS=false` 可由服务端
