...
因此此时必须将 MCP 视为完全可信代码。服务声明的工具说明和返回内容都按
不可信数据处理；新发现的自定义工具
没有内置风险元数据时默认按高风险进入现有规则、Reviewer、权限门控和哈希链
审计。Skill、MCP 描述或工具结果都不能授予权限；但管理员显式开启“完全访问”
后，调用会遵循该模式原有的不逐项确认语义。
...
未显式配置目录时，它们默认跟随数据库目录：`mcp-secrets/`、`skills/` 和
`skills-state.json`。生产部署应将三者放在只允许 KylinGuard 控制面账户访问的
持久目录；Docker 和 `install.sh` 已提供对应配置。
