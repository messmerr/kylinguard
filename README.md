...
- **麒麟环境感知**：联合 `nkvers`、发行版文件、内核、systemd、glibc 与 CPU 架构形成结构化平台画像，明确判断银河麒麟 V11 高级服务器版和 LoongArch64 证据是否完整。
...
- 银河麒麟版本、架构、原生组件和源码部署条件核验；
- systemd 根因证据、网络链路、块设备 I/O 与 LoongArch ELF 兼容检查；
- 主机与资源信息；
- systemd 服务管理；
- journal 日志查询；
- 网络连接与端口检查；
...
系统随包提供“银河麒麟环境核验”“systemd 服务根因分析”“网络分层诊断”“存储 I/O 根因分析”“LoongArch 程序兼容性诊断”和“磁盘空间诊断”六项内置 Skill。每项 Skill 都规定证据顺序、结论边界、停止条件和复核标准。自定义 stdio MCP 和 Skills 可在“扩展”页面添加、测试、启停；第三方工具仍须经过现有风险、权限和审计边界。
