...
- 后端 465 个 pytest 展开用例全绿，安全基准 60/60。
- 前端 40 个 Node 状态测试与生产构建通过，七个视图可用。
- 当前开发基线位于 `main`，会话权限与完整终端能力已经合入代码。
...
- **存储**：单个 SQLite 文件（WAL）保存审计链、会话、用户、策略与模型元数据；
  模型 API Key 单独保存在工作区外的受限文件中，SQLite 只存随机引用。
- **LLM**：GUI 管理 OpenAI / DeepSeek / DashScope / OpenAI 兼容提供商；
  规划与审查独立配置，会话可固定主模型与推理强度。模型管理复用登录态并写审计，
  不重复要求管理员密码；模型发现为 OpenAI 兼容接口提供
  `low` / `medium` / `high` 默认档位，
  并允许批量或逐模型覆盖。完全访问和高风险执行仍必须复验。

运行方式见 [README.md](README.md)。要点：`.env` 只需先配置
`KG_ADMIN_PASSWORD`，模型提供商、Key、可用模型和默认值全部登录后在
...
2. **独立 LLM 审查员**（`reviewer.py`）：与规划模型完全隔离的第二个 LLM，
   ...
   任何失败收敛为最高风险告警；Reviewer 提升确认强度，不替管理员作最终授权。
...
默认可用、开启时必须复验当前管理员密码，但不会自动获得 root，且后端重启后
...
- `App.vue`：登录门 + 工作区导航壳（六个工作区视图切换）。
- `views/`：`LoginView` / `ChatView`（对话运维）/ `ModelSettingsView`（模型服务）/
...
- `components/`：`Sidebar`（会话历史）/ `StatusPanel`（右侧实时状态）/
  ...
  `ConfirmCard`（确认卡）/ `MarkdownText`。
- `composables/`：`useChat.js`（SSE 事件聚合成回合制渲染模型）/
  `useModels.js`（提供商、默认值与会话模型状态）/
  `usePermissions.js`（权限模式/可信目录/授权）/ `useAuth.js`（鉴权封装）。
...
- **硬拒绝与权限请求分离**：无效工具参数/协议错误 hard deny；块设备、根目录
  破坏、控制面/敏感路径属于显著高风险，普通模式要求二次确认或拒绝，管理员
  复验开启的完全访问则不伪装成能力沙箱。
