...
- 后端 340+ 个 pytest 展开用例全绿，安全基准 60/60。
- 前端 Node 状态测试与生产构建通过，五个视图可用。
- 主线已合并至 `main`；权限系统在 `feature/permission-profiles` 开发分支收口。
...
2. **独立 LLM 审查员**（`reviewer.py`）：与规划模型完全隔离的第二个 LLM，
   ...
   任何失败收敛到"最不安全"。
3. **风险与权限门控**（`gate.py` + `authorization.py`）：硬红线不可覆盖；其余
   操作结合风险、会话模式、可信目录、一次/会话授权决定自动、询问或拒绝。
   高风险批准由后端重新验证管理员密码。
...
跳过逐项确认）。后两者有 TTL；完全访问必须由服务端开关启用、配置隔离执行账户
并复验当前管理员密码，永远不等于自动获得 root；独立 Reviewer 仍保留否决权。
...
| `command_batch.py` | 无 Shell 的简单复合命令解析与 argv 批处理 |
...
| `executor.py` | 受限命令执行（无 shell、进程组清理、有界双流输出、sudo -u 降权） |
...
- **正交分层**：门控决定"何时问人"（审批时机），执行器决定"技术上能做什么"
  （能力边界），互为纵深防御。
- **硬拒绝与权限请求分离**：格式不支持返回 rewrite，普通变更请求 permission，
  只有块设备、根目录破坏、控制面/敏感路径等保持不可越过的 hard deny。
...
- [x] **审计数据库与策略文件对执行账户不可访问**（控制面/执行面 OS 隔离）。
