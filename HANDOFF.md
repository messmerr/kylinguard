...
Agent 感知系统环境、规划执行计划、按全局审批模式与会话动作授权控制能力并全程留痕。
...
**M1 + M2 + 全局审批与会话动作授权系统已完成，正在收口部署迁移与交付材料。**

- 后端当前 634 个 pytest 用例在 WSL/Linux 环境全量通过。
- 前端当前 116 个 Node 状态测试与生产构建通过，七个视图可用。
- 当前扩展增强工作位于 `codex/custom-extensions`，尚未合并 `main`。

能演示的完整闭环：打开工作台 → 对话下发指令 → 实时看到感知/规划(流式)/三道闸校验/执行
各阶段 → 按全局审批模式自动执行或弹授权卡 → 审计回放中心按时间线回看整条哈希链
并校验完整性 → 权限与安全页管理全局自动执行范围/会话授权/高级规则 → 仪表盘看全局统计。
...
2. **独立 LLM 审查员**（`reviewer.py`）：与规划模型完全隔离的第二个 LLM，
   ...
   风险告警会提升确认强度；Reviewer 不可用时自动审核降级为人工确认，且不替用户
   作最终授权。
3. **风险与权限门控**（`gate.py` + `authorization.py`）：空输入、NUL、非法 argv
   等执行协议错误不可覆盖；其余操作结合风险、审批模式、自动执行范围、一次/会话
...
`auto_review`（审核通过的可逆操作在授权范围内自动执行）/ `full_access`（执行账户权限内
启用完整 shell、文件、网络与进程能力且不逐项确认）。完全访问不再自动到期；其入口
默认隐藏，只能在“权限与安全”中经独立警告和二次输入确认后显示，真正启用时再经过
另一组独立警告和二次输入确认。后端拒绝在入口隐藏时直接启用；重新隐藏会原子收回
正在生效的完全访问。完全访问不会自动获得 root，且后端重启后
总会收回。`KG_EXEC_USER` 可选；留空使用后端当前 OS 身份。不同 UID 仅代表
执行账户分离，控制面是否真正隔离仍由文件权限/ACL 决定。

审批模式、自动执行目录和完全访问状态使用全局单例配置，对现有及以后创建的所有
任务立即生效。只有“允许一次”和“本次会话允许同类操作”属于会话动作授权。
每个会话仍持久化独立 `workspace_root`：新任务可先选择后端/WSL 可见目录，终端调用
默认注入该 `cwd`，规划上下文也会明确告知模型；它不是 OS 沙箱。
...
| `sessions.py` | 会话元数据、全局权限持久化与会话授权执行租约 |
| `permissions.py` | 全局权限上下文、待决会话授权、TTL 与版本控制 |
| `authorization.py` | 工具调用能力描述、自动审核范围与审批模式裁决 |
...
- `composables/`：`useChat.js`（SSE 事件聚合成回合制渲染模型）/
  ...
  `usePermissions.js`（审批模式/自动执行范围/授权）/ `useApi.js`（请求封装）。
